...
beforeEach(function () {
    ...
    $this->rule = Rule::factory()->create(['project_id' => $this->project->id, 'rule_id' => 'test-rule']);
...
test('log output is always saved to agent_runs.output by default', function () {
    ...
    $this->handler->handle($this->agentRun, $this->rule, []);
...
test('no output config does nothing', function () {
    Process::fake();
...
test('both github_comment and github_reaction can be configured together', function () {
    ...
    // Should run both comment and reaction
    Process::assertRan(function ($process) {

...
new #[Title('Config Editor')] class extends Component {
    ...
    public function deployConfig(): void
    {
        ...
        // Create a branch and commit, then open a PR
        $branchName = 'dispatch/config-update-'.now()->format('Ymd-His');
        $installationId = $installation->installation_id;

        // Get the default branch ref SHA
        $defaultBranch = $this->getDefaultBranch($project->repo, $installationId);
        if (! $defaultBranch) {
            $this->errorMessage = 'Failed to detect default branch.';
            return;
        }

        $refSha = $this->getBranchSha($project->repo, $defaultBranch, $installationId);
        if (! $refSha) {
            $this->errorMessage = 'Failed to read default branch ref.';
            return;
        }

        // Create the new branch
        if (! $this->createBranch($project->repo, $branchName, $refSha, $installationId)) {
        ...
        if ($prUrl) {
            $this->statusMessage = "Pull request created: {$prUrl}";
        } else {
            $this->statusMessage = "Config committed to branch {$branchName} but PR creation failed.";
        }
    }

    protected function getDefaultBranch(string $repo, int $installationId): ?string
    {
        try {
            $response = $this->githubRequest($installationId)
    ...
    protected function getBranchSha(string $repo, string $branch, int $installationId): ?string
    {
        try {
            $response = $this->githubRequest($installationId)
                ->get("https://api.github.com/repos/{$repo}/git/ref/heads/{$branch}");

            return $response->successful() ? $response->json('object.sha') : null;
        } catch (\Throwable) {
            return null;
        }
    }

    protected function createBranch(string $repo, string $branch, string $sha, int $installationId): bool
    {
        try {
            $response = $this->githubRequest($installationId)
    ...
    protected function createPullRequest(string $repo, string $head, string $base, int $installationId): ?string
    {
        try {
            $response = $this->githubRequest($installationId)

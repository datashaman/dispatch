...
new #[Title('Config Editor')] class extends Component {
    ...
    public function deployConfig(): void
    {
        ...
        if (! $installation) {
            $this->errorMessage = 'No GitHub App installation linked to this project.';
            return;
        }

        // Read the current dispatch.yml from disk
        $filePath = rtrim($project->path, '/').'/dispatch.yml';
        if (! file_exists($filePath)) {
            $this->errorMessage = 'No dispatch.yml found. Save first.';
            return;
        }

        $content = file_get_contents($filePath);
        if ($content === false) {
            $this->errorMessage = 'Failed to read dispatch.yml.';
            return;
        }
        ...
        $repoInfo = $this->githubApi('get', "{$apiBase}/repos/{$project->repo}", $installationId);
        if (! $repoInfo['ok']) {
            $this->errorMessage = "Failed to fetch repo info: {$repoInfo['error']}";
            return;
        }
        $defaultBranch = $repoInfo['data']['default_branch'] ?? 'main';

        // Get the default branch ref SHA
        $refInfo = $this->githubApi('get', "{$apiBase}/repos/{$project->repo}/git/ref/heads/{$defaultBranch}", $installationId);
        if (! $refInfo['ok']) {
            $this->errorMessage = "Failed to read branch ref: {$refInfo['error']}";
            return;
        }
        $refSha = $refInfo['data']['object']['sha'] ?? null;
        if (! $refSha) {
            $this->errorMessage = 'Failed to read branch SHA from ref.';
            return;
        }

        // Create the new branch
        $branchName = 'dispatch/config-update-'.now()->format('Ymd-His');
        $branchResult = $this->githubApi('post', "{$apiBase}/repos/{$project->repo}/git/refs", $installationId, [
            'ref' => "refs/heads/{$branchName}",
            'sha' => $refSha,
        ]);
        if (! $branchResult['ok']) {
            $this->errorMessage = "Failed to create branch: {$branchResult['error']}";
            return;
        }

        // Commit the file to the new branch
        $client = app(GitHubApiClient::class);
        $result = $client->commitFile(
            $project->repo,
            'dispatch.yml',
            $content,
            'chore: update dispatch.yml via Config Editor',
            $installationId,
            $branchName,
        );
        ...
        $prResult = $this->githubApi('post', "{$apiBase}/repos/{$project->repo}/pulls", $installationId, [
            'title' => 'chore: update dispatch.yml via Config Editor',
            'head' => $branchName,
            'base' => $defaultBranch,
            'body' => "Updated `dispatch.yml` from the Dispatch Config Editor.\n\n🤖 Generated by [Dispatch](https://github.com/datashaman/dispatch)",
        ]);

        if ($prResult['ok']) {
            $prUrl = $prResult['data']['html_url'] ?? null;
            $this->statusMessage = $prUrl
                ? "Pull request created: {$prUrl}"
                : "Config committed to branch {$branchName}.";
        } else {
            $this->statusMessage = "Config committed to branch {$branchName} but PR creation failed: {$prResult['error']}";
        }
    }
     ...
     */
    protected function githubApi(string $method, string $url, int $installationId, array $data = []): array
    {
        try {
            $request = $this->githubRequest($installationId);
            ...
            if (! $response->successful()) {
                ...
                \Illuminate\Support\Facades\Log::warning("GitHub API {$method} {$url} failed", [
                    'status' => $response->status(),
                    'message' => $msg,
                ]);

                return ['ok' => false, 'data' => null, 'error' => "({$response->status()}) {$msg}"];
            }

            return ['ok' => true, 'data' => $response->json(), 'error' => null];
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error("GitHub API {$method} {$url} exception", [
                'error' => $e->getMessage(),
            ]);
    ...
    protected function githubRequest(int $installationId): \Illuminate\Http\Client\PendingRequest
    {
        $token = app(\App\Services\GitHubAppService::class)->getInstallationToken($installationId);
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-2">
            ...
            @if (! empty($configData))
                ...
                @if ($this->project->githubInstallation)
                    <flux:button
                        variant="ghost"
                        icon="arrow-up-tray"
                        wire:click="deployConfig"
                        wire:loading.attr="disabled"
                        wire:confirm="This will create a branch and PR with the current dispatch.yml in {{ $this->project->repo }}. Continue?"
                    >
                        <span wire:loading.remove wire:target="deployConfig">
                            {{ __('Deploy to Repo') }}
                        </span>
                        <span wire:loading wire:target="deployConfig">{{ __('Deploying…') }}</span>
                    </flux:button>
                @endif
            @endif
        </div>
